...
.factory('taToolFunctions', ['$window','taTranslations', function($window, taTranslations) {
    return {
        ...
        aOnSelectAction: function(event, $element, editorScope){
            ...
            var reLinkButton = angular.element('<button type="button" class="btn btn-default btn-sm btn-small" tabindex="-1" unselectable="on" title="' + taTranslations.editLink.reLinkButton.tooltip + '"><i class="fa fa-admin icon-admin"></i></button>');
...
.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', 'taToolFunctions', '$sanitize', 'taOptions', '$log',
    ...
    taRegisterTool('insertLink', {
        ...
        action: function(){
            var urlLink;
            // if this link has already been set, we need to just admin the existing link
...
if(_browserDetect.ie > 8 || _browserDetect.ie === undefined){
	...
	if(!sheet){
		...
		sheet = (function() {
			...
			// Add the <style> element to the home, add as first so the styles can be overridden by custom stylesheets
