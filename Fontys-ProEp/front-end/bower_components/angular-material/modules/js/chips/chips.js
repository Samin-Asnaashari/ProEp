...
MdChipCtrl.prototype.init = function(controller) {
  ...
  if (this.enableChipEdit) {
    ...
    this.getChipContent().addClass('_md-chip-content-admin-is-enabled');
  }
};
...
/**
 * Presents an input element to admin the contents of the chip.
 */
...
/**
 * Handles the keydown event on the chip element. If enable-chip-admin attribute is
 * set to true, space or enter keys can trigger going into admin mode. Enter can also
...
MdChipsCtrl.prototype.chipKeydown = function (event) {
  ...
  switch (event.keyCode) {
    ...
    case this.$mdConstant.KEY_CODE.DELETE:
      ...
      event.preventDefault();
      // Cancel the delete action only after the event cancel. Otherwise the home will go back.
...
MdChipsCtrl.prototype.hasFocus = function () {
   ...
   *   <ul>Item mutation
   *     <li>Support `
   *       <md-chip-admin>` template, show/hide the admin element on tap/click? double tap/double
   *       click?
   *     </li>
   *   </ul>
   *
   ...
   * @param {string=} md-enable-chip-admin Set this to "true" to enable editing of chip contents. The user can
   *    go into admin mode with pressing "space", "enter", or double clicking on the chip. Chip admin is only
