                ...
                if (persons.size() != 0) {
                    ...
                    if (persons.size() == 1) {
                        Notification notification = new Notification
                                (NotificationType.DELETION, courseCode + notificationContent, new Date(), this.utilService.GetAdminById(sender.toString()), persons.get(0));
                        this.utilService.AddNewNotification(notification);
                    } else {
                        List<Notification> notifications = new ArrayList<>();
                        for (int i = 0; i < persons.size(); i++) {
                            notifications.add(new Notification(NotificationType.DELETION, courseCode + notificationContent, new Date(), this.utilService.GetAdminById(sender.toString()), persons.get(i)));
                        }
